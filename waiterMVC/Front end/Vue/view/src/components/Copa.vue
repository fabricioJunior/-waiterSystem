  ...
  <div>
    ...
    <div>
      <table id="customers">
        ...
        <tr v-for="b in bebidas" :key="b.id" >
           <td>{{b.idPedido}}</td>
           <td>{{b.Pedido.mesa}}</td>
           <td>{{b.Menu.nome}}</td>
    ...
    <input class = "input" type="submit" value="Atualizar Bebidas Prontas" @click="atualizarBebidas"/>
    <input class="input" type="submit" value="Atualizar Bebidas" @click="atualizar">
  </div>
</template>
<style scoped>
...
export default {
  ...
  methods: {
    atualizarBebidas:function(){
       let array = Array.from(this.bebidas);
       ...
       console.log(prontas);
       if(prontas.length >= 1){
          axios.post(url + 'copa',prontas).then( d =>{
             console.log(d);
             this.atualizar();
          } ).catch(erro =>{
          console.log(erro);
